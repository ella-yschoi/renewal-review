...
> "자가 수정 루프는 3회 반복 만에 기능 구현 + 품질 검증 + 의도 검증을 완료했다.
> 사람 개입 0회, 수동 대비 40% 시간 절약.
> Phase 2(ruff/pytest)에서 1회, Phase 3(삼각 검증)에서 1회 실패 후 자동 수정.
> **기능을 PROMPT.md에 정의하면, 검증된 코드가 나오는 파이프라인.**"
...
- [ ] PROMPT.md 작성 (과제 + 완료 기준)
- [ ] requirements.md 작성 (삼각 검증용)
- [ ] scripts/triangular-verify.sh 작성 + 테스트
- [ ] scripts/self-correcting-loop.sh 작성 + 테스트
- [ ] 자동 루프 실행 — 완료까지 기록
- [ ] 대조군(수동) 실행 — 동일 과제
- [ ] 기록 템플릿 채우기
