...
DB → load_pairs() → [RenewalPair]
            ...
            process_pair() ◄──┘ (loop)
                  │
        compute_diff(pair) → DiffResult (변경 사항)
                  │
        flag_diff(diff, pair) → DiffResult (DiffFlag 부여)
   ...
   analyze_pair() → [LLMInsight]                    ReviewResult (rule only)
        │
   aggregate(rule_risk, insights) → risk 상향 가능
        │
   generate_summary(result) → AI summary (optional)
        ...
        └──────────────────┬───────────────────────┘
              ...
              writer.save_rule_result()     (DB persist)
                           │
              history_store ← BatchRunRecord (추세 분석용)
```

#### Lazy Enrichment (개별 조회 시)

```
GET /reviews/{pn} → review_store.get(pn)
                         │
           ┌── summary 없고 LLM client? ──┐
           │ Yes                           │ No
    analyze_pair() + generate_summary()    그대로 반환
           │
...
POST /quotes/generate → process_pair() → generate_quotes(pair, diff)
                                    ...
                                    ┌── LLM client? ──┐
                                    │ Yes              │ No
                             personalize_quotes()    rule-based quotes만
                                    │
                           [QuoteRecommendation]
```
